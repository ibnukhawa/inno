...
class InnoSerialNumber(models.Model):
	_name = 'inno.serial.number'
	...
	part_request_ids = fields.One2many('inno.part.request.serial', 'part_request_id', string="Material Request Line")
	...
	def default_get(self, fields):
		res = super(InnoSerialNumber, self).default_get(fields)
    ...
    #     seq = self.env['ir.sequence'].get('inno.serial.number') 
    #     vals['name'] = seq
    #     result = super(InnoSerialNumber,self).create(vals)
    #     return result
	...
	def create(self, vals):
		...
		# 	vals['name'] = self.env['ir.sequence'].next_by_code('inno.serial.number') or _('New')
		# res = super(InnoSerialNumber, self).create(vals)
	...
	def action_fill_part_request_lines(self):
		for req in self:
			part_line_obj = self.env['inno.part.request.serial']
	...
	@api.multi
	def write(self, vals):
		res = super(InnoSerialNumber, self).write(vals)
	...
	# 	return super(InnoSerialNumber, self).unlink()


class InnoPartRequestSerial(models.Model):
	_name = 'inno.part.request.serial'

	part_request_id = fields.Many2one('inno.serial.number', string="Material Request")
	sale_id = fields.Many2one('sale.order', string='SO Reference')	
